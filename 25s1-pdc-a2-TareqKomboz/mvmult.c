 ...
 *    StudentID: A1976365
 *
 *    MPI_Wtime() returns the elapsed wall-clock time in seconds since an arbitrary point in the past.
 *    MPI_Barrier(MPI_COMM_WORLD) ensures all ranks start and end the timing together.
 *    We measure the time after all processes have finished, so we capture the slowest one.
 *    Only rank 0 prints the elapsed time — which reflects the total runtime from the perspective of the whole program.
 *
...
#  endif
   ...
   MPI_Reduce(&loc_elapsed, &elapsed, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD); // Get maximum local elapsed time across all processes (total program runtime)

   // Only rank 0 prints elapsed time.
   if (my_rank == 0) {
